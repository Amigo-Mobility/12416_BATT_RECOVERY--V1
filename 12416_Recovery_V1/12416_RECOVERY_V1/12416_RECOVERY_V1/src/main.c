 ...
 * /////////////////////////////////////////////////////////////////////////////////////////////////////////////
 * Prem- 9-6-2024
 * Battery recovery mode added
 * 15% or less would pop up footer message- Contact store manager
 * it would disable drive as long as battery reads 15% or less
 * recovery would require min of 8 min horn button pressed
 * uploaded to github
 * //////////////////////////////////////////////////////////////////////////////////////////////////////////////
 * 
 * 
...
typedef enum
{
    ...
    FOOTER_BRAKERLS_OPEN = 7,  FOOTER_LOWCHARGE = 8,FOOTER_CRITICALLY_LOWCHARGE=9
}FOOTER_TYPE;
...
GFX_XCHAR CRITICALLY_LOW_BATT_TEXT[] = {"CONTACT STORE MANAGER"};
unsigned int FLAG_RECOVERY_MODE=0;
 unsigned int DELAY_RECOVERY_MODE=0;
...
//            {}
         ...
         else if(batt_level>15 && batt_level <= 65)                        //Charge level <= 65% diplay CHARGE SOON
         ...
         else if(batt_level<20)                        //Charge level <= 15% critically low state diplay CHARGE SOON

            {
                curr_footer = FOOTER_CRITICALLY_LOWCHARGE;//5;
                if(curr_footer != prev_footer)
                {
                    GFX_ColorSet(FOOTER_CLR_RED);//(253,238,57));   //yellow
                    APP_WaitUntilFinished(GFX_RectangleFillDraw(footer_topleft_x,footer_topleft_y,footer_bottomright_x,footer_bottomright_y));
                    ...
                    GFX_FontSet(UI_FOOTER_FONT);
                    //GFX_FontSet(UI_FOOTER_FONT_3);
                    APP_WaitUntilFinished(GFX_TextStringBoxDraw(footer_text_top_x,footer_text_top_y,319,footer_text_height,CRITICALLY_LOW_BATT_TEXT ,0, GFX_ALIGN_CENTER));
                }
            }   
...
void UI_state(uint8_t fc, volatile _ulta_wa * cs_data, uint8_t GPS_state)//uint8_t batt, uint8_t speed)
{
    ...
    static TIMER_TICK prevTick; 
     batt_perc = calc_battery_perc(cs_data->aux_data);
...
int main( void )
{
    ...
    bool deb_horn_in = 1;
    TIMER_TICK _timer_main = 0, _timer_i2c = 0, _timer_main2 = 0,_timer_recovery = 0; 
   FLAG_RECOVERY_MODE= 0;
...
//            }
   ...
   printf("CM %d",CS1108_ultaStatus.misc_data.bits.chg_mode);
    ...
    else//else turn On
    {
        ...
        curr_pwr_state  = DISP_ON;
        //CS1108_Buttonstatus.byte &= ~(!DM_INDOOR<<ULTA_DRIVEMODE_BIT);   //Clear 6th bit stop unit from runing  
          // CS1108_ultaStatus.misc_data.byte &= ~(!DM_INDOOR<<4); 
        CS1108_Buttonstatus.byte &= ~(!DM_INDOOR<<ULTA_QSTOP_BIT);
        FLAG_RECOVERY_MODE= 0;
...
//   }
   ...
   if ((FLAG_INFO_DISPLAYED) && (check_horn()))
   {
     ...
     {
        ClrWdt(); //Clear watchdog
         if(TIMER_Since(_timer_main)> 30000)//If no user button press for more than 2 seconds move to disclaimer.
        {
            clear_counters();
            continue;
        }
  ...
  //      update_driveMode(curr_GPS_state);
     ...
     while( 1 )
    {
        ...
        if((ui_state != sleeping)&&(curr_pwr_state != DISP_OFF))
        {
         ...
         if(FLAG_RECOVERY_MODE==0 || TIMER_Since( _timer_recovery) > 1000){
        ...
        if(batt_perc<20){
            ...
            if(FLAG_RECOVERY_MODE==0){
              ...
              // CS1108_ultaStatus.misc_data.byte &= ~(!DM_INDOOR<<4); 
                    ...
                    delay_with_Revhorn(&horn_state_isr, &deb_horn_in, 100);
                  FLAG_RECOVERY_MODE= 1;
                  DELAY_RECOVERY_MODE=0;
                  
                }
            
            
            else if(FLAG_RECOVERY_MODE==1){
                
           
                   if(check_horn()){
                      // while(check_horn()){
                            DELAY_RECOVERY_MODE=DELAY_RECOVERY_MODE + 1;
                            delay_with_Revhorn(&horn_state_isr, &deb_horn_in, 100);
                   
                       
                      
                       
                         if(DELAY_RECOVERY_MODE>5){
                                FLAG_RECOVERY_MODE= 2;
                                DELAY_RECOVERY_MODE=0;
                               // CS1108_Buttonstatus.byte |= DM_OUTDOOR<<ULTA_DRIVEMODE_BIT;  //set 6th bit go to normal mode
                                 CS1108_Buttonstatus.byte |= DM_OUTDOOR<< ULTA_QSTOP_BIT;  //set 6th bit go to normal mode
                              //  CS1108_ultaStatus.misc_data.byte |= DM_OUTDOOR<< 4; 
                               
                                 // update_driveMode(curr_GPS_state); for st feature
                                delay_with_Revhorn(&horn_state_isr, &deb_horn_in, 100);
                         }
        ...
        else
        {
            ...
            CS1108_Buttonstatus.byte |= DM_OUTDOOR<< ULTA_QSTOP_BIT; 
            FLAG_RECOVERY_MODE= 0;
            
        }
           // update_driveMode(curr_GPS_state); //for st feature
         }//end of timer for 1000 for this loop   
       ...
       //  uint8_t batt_perc1 = calc_battery_perc(CS1108_ultaStatus->aux_data);
